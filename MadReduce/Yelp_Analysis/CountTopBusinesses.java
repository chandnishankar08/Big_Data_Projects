...
public class CountTopBusinesses {

    public static void main(String[] args) throws Exception {
        Configuration conf = new Configuration();
        String[] otherArgs = new GenericOptionsParser(conf, args).getRemainingArgs();
        if (otherArgs.length != 2) {
            System.err.println("Usage: CountTopBusinesses <in> <out>");
            System.exit(2);
        }
    ...
    public static class TopNMapper extends Mapper<Object, Text, Text, IntWritable> {
        ...
        public void map(Object key, Text value, Context context) throws IOException, InterruptedException {
			...
			if (businessData.length ==3) {
				String[] address = StringUtils.split(businessData[1].toString(), ",");
    ...
    /**
     * The reducer retrieves every zipcode and puts it into a Map: if the word already exists in the
    ...
    public static class TopNReducer extends Reducer<Text, IntWritable, Text, IntWritable> {
        ...
        public void reduce(Text key, Iterable<IntWritable> values, Context context) throws IOException, InterruptedException {
            ...
            // computes the number of occurrences of a single zipcode
            int sum = 0;
            for (IntWritable val : values) {
                sum += val.get();
            }

            // puts the number of occurrences of this zipcode into the map.
            // We need to create another Text object because the Text instance
            // we receive is the same for all the zipcodes
    ...
    /**
     * The combiner retrieves every zipcode and puts it into a Map: if the word already exists in the
     * map, increments its value, otherwise sets it to 1.
     */
